...
- **S4P-OQ3 (new):** When is the precondition-risk sort *insufficient*? Specifically, the heuristic is overcautious — it takes the max `del_effects` across *all* operators that could achieve a precondition, even ones the actual sub-plan won't use. Find a forcing scene where this over-estimation defers the wrong precondition and the resulting plan is suboptimal.
