...
|---|---|---|
| P1 | ≥ 80% approved, 0 rejected across ≥ 7 prose reviews. Sketch-04 grows reviewable prose from 7 fields to 12 (+3 chain-step annotations — all three chain members carry non-empty prose: 2 Hamlet staging steps + 1 Claudius parallel step — + 2 arc-relation annotations); prediction is on proportion. | PASS expected. Session 5 was 7/7 approved. |
...
- **OQ-AP13 — reviewable-prose scope creep.** APA4-3's decision to thread arc-relation and staging-step annotations through as reviewable prose grows the reviewable-field count from 7 to 12 on Hamlet (all three chain members carry prose; implementation confirmed this at test time — initial sketch count of 11 undercounted Claudius's parallel step). If Session 6's prose-review quality holds (P1 passes at the new size), no change; if it degrades (approval proportion drops or reviews thin out), the sketch-04 decision may have been over-aggressive and a future sketch may narrow reviewable-prose back to ArMythos / ArPhase / ArCharacter.
