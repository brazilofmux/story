"""
demo_dramatic_reader_model_macbeth_template.py — invoke the cross-boundary
reader-model probe on the Macbeth dramatica-complete encoding.

Second Template-surface probe run (after Rocky's 2026-04-17 landing).
Macbeth has a very different substrate profile from Rocky: 1 terminal
retraction + 3 mid-arc rule-emergences (kinslayer, regicide,
breach_of_hospitality emerge at the killing events' τ_s; tyrant
emerges at the crowning). DSP_limit landed APPROVED 0.67 pre- and
post-sketch-02 (middle-arc kinds = 1 via rule-emergence).

The probe is invited to comment on the Macbeth verifier's verdicts
(9 checks, all APPROVED in the latest run) and the Dramatic/Template/
Lowering surface. Interesting signals to watch for:

- Does the probe endorse the cumulative-degradation-as-Optionlock
  reading, or propose a richer characterization?
- Does the probe catch any annotation-quality issues below verifier
  granularity (parallel to Rocky's L_mc_throughline needs-work)?
- Does the probe identify any further predicate weaknesses in the
  9-check surface?

Usage:
    cd prototype
    export ANTHROPIC_API_KEY=...
    .venv/bin/python3 demo_dramatic_reader_model_macbeth_template.py

    # Dry run (no API call, prints the full prompt):
    .venv/bin/python3 demo_dramatic_reader_model_macbeth_template.py --dry-run

    # Save the full structured result as JSON:
    .venv/bin/python3 demo_dramatic_reader_model_macbeth_template.py \\
        --save-json reader_model_macbeth_dramatica_complete_output.json
...
from macbeth import ENTITIES, FABULA
...
from macbeth_dramatica_complete_verification import run as run_verifier
...
def main() -> int:
    ...
    print("Cross-boundary reader-model probe — Macbeth (dramatica-complete Template surface)")
