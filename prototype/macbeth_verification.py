...
def _entity_id_for_character(character_id: str, lowerings: tuple) -> str:
    ...
    Dramatic Character lowers to. Same logic as
    oedipus_verification's helper — duplicated rather than imported
    because the Lowerings-set differs per encoding (LOWERINGS here is
    macbeth_lowerings.LOWERINGS); the helper closes over which
    Lowerings to walk."""
