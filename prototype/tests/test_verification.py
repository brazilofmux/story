...
def test_coupling_kind_for_field_lookup():
    from story_engine.core.dramatic import coupling_kind_for
...
def test_declarations_for_kind_returns_only_matching():
    from story_engine.core.dramatic import declarations_for_kind
...
def test_fields_with_coupling_for_argument():
    from story_engine.core.dramatic import fields_with_coupling
...
def test_oedipus_verification_main_character_check_passes():
    ...
    import story_engine.encodings.oedipus_verification as ov
    results = ov.run()
...
def test_orchestrate_oedipus_run_matches_three_results():
    ...
    from story_engine.encodings.oedipus_verification import run as run_oedipus
    out = run_oedipus()
    assert len(out) == 3
...
def test_orchestrate_macbeth_run_matches_three_results():
    ...
    from story_engine.encodings.macbeth_verification import run as run_macbeth
    out = run_macbeth()
    assert len(out) == 3
    reviews = reviews_only(out)
    assert len(reviews) == 3
...
def test_orchestrate_ackroyd_run_matches_three_results():
    ...
    from story_engine.encodings.ackroyd_verification import run as run_ackroyd
    out = run_ackroyd()
    assert len(out) == 3
    reviews = reviews_only(out)
    assert len(reviews) == 3
...
def test_orchestrate_macbeth_save_the_cat_run_matches_four_results():
    ...
    from story_engine.encodings.macbeth_save_the_cat_verification import run as run_mstc
    out = run_mstc()
    assert len(out) == 4
    reviews = reviews_only(out)
...
def test_orchestrate_ackroyd_save_the_cat_run_matches_four_results():
    ...
    reviews."""
    from story_engine.encodings.ackroyd_save_the_cat_verification import run as run_astc
    out = run_astc()
    assert len(out) == 4
    reviews = reviews_only(out)
    assert len(reviews) == 4
...
def test_coverage_report_macbeth_save_the_cat_surfaces_beat_gaps():
    ...
    from story_engine.encodings.macbeth_save_the_cat_verification import (
        CHECK_REGISTRY, RECORDS_BY_TYPE,
    )
    from story_engine.core.save_the_cat import COUPLING_DECLARATIONS as STC_DECLS
...
def test_coverage_report_macbeth_surfaces_gaps_against_real_encoding():
    ...
    from story_engine.encodings.macbeth_verification import CHECK_REGISTRY, RECORDS_BY_TYPE
    from story_engine.encodings.macbeth_dramatic import BEATS, STAKES
    from story_engine.core.dramatic import COUPLING_DECLARATIONS
...
def _classifier_fixtures():
    ...
    from story_engine.core.verifier_helpers import classify_event_action_shape
    return (
        Event, KnowledgeEffect, WorldEffect, Held, Prop, Slot,
...
def _mk_ke(agent_id, predicate, *args):
    from story_engine.core.substrate import KnowledgeEffect, Held, Prop, Slot, Confidence
...
def _mk_we(predicate, *args):
    from story_engine.core.substrate import WorldEffect, Prop
...
def test_ek2_oedipus_verifier_both_characterizations_approved():
    ...
    from story_engine.encodings.oedipus_dramatica_complete_verification import run
    reviews = run()
...
def test_ek2_macbeth_verifier_no_regression():
    ...
    from story_engine.encodings.macbeth_dramatica_complete_verification import run
    reviews = run()
    by_target = {r.target_record.record_id: r for r in reviews}
...
def test_ek2_ackroyd_verifier_be_er_inversion_preserved():
    ...
    from story_engine.encodings.ackroyd_dramatica_complete_verification import run
    reviews = run()
    by_target = {r.target_record.record_id: r for r in reviews}
...
def test_verifier_surface_has_eight_checks_per_encoding():
    ...
    checks today."""
    from story_engine.encodings.oedipus_dramatica_complete_verification import run as o_run
    from story_engine.encodings.macbeth_dramatica_complete_verification import run as m_run
    from story_engine.encodings.ackroyd_dramatica_complete_verification import run as a_run
    assert len(o_run()) >= 8
    assert len(m_run()) >= 8
    assert len(a_run()) >= 8
...
def _lt_fixtures():
    ...
    from story_engine.core.verifier_helpers import classify_arc_limit_shape
    return (
        Event, KnowledgeEffect, WorldEffect, Held, Prop, Slot,
        Confidence, Rule, Branch, IDENTITY_PREDICATE,
...
def _lt_branches():
    from story_engine.core.substrate import Branch
...
def test_lt2_identity_resolution_via_world_effect():
    ...
    at fold time."""
    from story_engine.core.substrate import WorldEffect, Prop, IDENTITY_PREDICATE
    (*_, classify) = _lt_fixtures()
    canonical, all_branches = _lt_branches()
...
def test_lt2_rule_emergence_signal_alone():
    ...
    from story_engine.core.substrate import WorldEffect, Prop, Rule
    (*_, classify) = _lt_fixtures()
    canonical, all_branches = _lt_branches()
...
def test_lt2_oedipus_fabula_classifies_optionlock():
    ...
    from story_engine.core.verifier_helpers import classify_arc_limit_shape
    from story_engine.encodings.oedipus import FABULA, RULES, ALL_BRANCHES
...
def test_lt2_macbeth_fabula_classifies_optionlock():
    ...
    from story_engine.core.substrate import CANONICAL
    from story_engine.core.verifier_helpers import classify_arc_limit_shape
    from story_engine.encodings.macbeth import FABULA, RULES, ALL_BRANCHES
...
def test_lt2_ackroyd_fabula_classifies_optionlock():
    ...
    from story_engine.core.substrate import CANONICAL
    from story_engine.core.verifier_helpers import classify_arc_limit_shape
    from story_engine.encodings.ackroyd import FABULA, RULES, ALL_BRANCHES
    classification, _strength, signals = classify_arc_limit_shape(
        FABULA, RULES, CANONICAL, ALL_BRANCHES,
    )
    assert classification == "optionlock"
...
def test_lt2_rocky_substrate_shows_mild_convergence_not_clean_timelock():
    ...
    from story_engine.core.substrate import CANONICAL
    from story_engine.core.verifier_helpers import classify_arc_limit_shape
    from story_engine.encodings.rocky import FABULA, RULES, ALL_BRANCHES
    classification, strength, signals = classify_arc_limit_shape(
        FABULA, RULES, CANONICAL, ALL_BRANCHES,
    )
    assert classification == "optionlock"
...
def test_rocky_went_the_distance_rule_derives_at_fight_end():
    ...
    from story_engine.encodings.rocky import FABULA, RULES, ALL_BRANCHES, went_the_distance
...
def test_rocky_has_no_identity_propositions():
    ...
    from story_engine.core.substrate import IDENTITY_PREDICATE, WorldEffect, KnowledgeEffect
    from story_engine.encodings.rocky import FABULA, ENTITIES
...
def test_lt5_timelock_declared_on_timelock_consistent_substrate_is_noted():
    ...
    from story_engine.core.substrate import CANONICAL, Branch
    canonical = Branch(label=":canonical", kind="canonical", parent=None)
...
def test_lt5_optionlock_declared_on_optionlock_substrate_is_approved():
    ...
    present → APPROVED."""
    from story_engine.core.verifier_helpers import dsp_limit_characterization_check
    from story_engine.core.substrate import CANONICAL, WorldEffect, Prop, Branch, Event
    canonical = Branch(label=":canonical", kind="canonical", parent=None)
    all_branches = {":canonical": canonical}
    P = Prop("x", ("a",))
    e1 = Event(
...
def test_rocky_verifier_has_nine_checks():
    ...
    from story_engine.encodings.rocky_dramatica_complete_verification import run
    assert len(run()) == 9
...
def _make_event_at(τ_s, event_id, prop, asserts=True):
    ...
    from story_engine.core.substrate import Event, WorldEffect
    return Event(
...
def test_lt7_peripheral_pre_retraction_does_not_count_as_middle_arc():
    ...
    from story_engine.core.substrate import Branch, Prop
    canonical = Branch(label=":canonical", kind="canonical", parent=None)
    all_branches = {":canonical": canonical}
    P = Prop("x", ("a",))
    fabula = (
...
def test_lt12b_subject_reactivation_retraction_is_enabling():
    ...
    from story_engine.core.verifier_helpers import classify_arc_limit_shape_strong
    from story_engine.core.substrate import Branch, Prop, Event, WorldEffect
    canonical = Branch(label=":canonical", kind="canonical", parent=None)
    all_branches = {":canonical": canonical}
...
def test_lt12_preserves_prior_optionlock_verdicts():
    ...
    from story_engine.encodings.macbeth_dramatica_complete_verification import (
        run as macbeth_run,
    )
    from story_engine.encodings.oedipus_dramatica_complete_verification import (
        run as oedipus_run,
    )
    from story_engine.encodings.ackroyd_dramatica_complete_verification import (
        run as ackroyd_run,
    )
...
def test_ag_returns_none_when_mc_not_participant():
    ...
    from story_engine.core.verifier_helpers import classify_event_agency_shape
    from story_engine.core.substrate import Event, WorldEffect, Prop
    e = Event(
...
def test_re5_compute_pre_post_ratios_detects_shift():
    ...
    from story_engine.core.verifier_helpers import compute_pre_post_action_ratios
    from story_engine.core.substrate import Event, WorldEffect, Prop
...
def test_rr2_detect_preceding_ic_event_correlation_fires():
    ...
    value."""
    from story_engine.core.verifier_helpers import detect_preceding_ic_event
...
def test_rr4_events_advancing_throughline_finds_oedipus_jocasta_events():
    ...
    events."""
    from story_engine.core.verifier_helpers import events_advancing_throughline
...
def test_bw2_beat_type_weight_canonical_values():
    ...
    declared vocabulary."""
    from story_engine.core.verifier_helpers import beat_type_weight
...
def test_bw3_event_to_beat_type_resolves_through_scene_advance():
    ...
    from story_engine.core.verifier_helpers import event_to_beat_type
    from story_engine.encodings.macbeth_dramatic import SCENES, BEATS
...
def test_mn_returns_none_when_mc_not_participant():
    ...
    from story_engine.core.verifier_helpers import classify_event_manipulation_shape
    from story_engine.core.substrate import Event, WorldEffect, Prop
    e = Event(
...
def test_ig_oedipus_story_goal_uses_knowledge_projection_not_world_projection():
    ...
    from story_engine.core.substrate import CANONICAL, in_scope, project_world, project_knowledge, holds_derived
    from story_engine.encodings.oedipus import FABULA, ALL_BRANCHES, RULES, parricide, killed, child_of
    events = [e for e in FABULA if in_scope(e, CANONICAL, ALL_BRANCHES)]
