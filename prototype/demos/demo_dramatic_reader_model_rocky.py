"""
demo_dramatic_reader_model_rocky.py — invoke the cross-boundary
reader-model probe on the Rocky dramatica-complete encoding.

First demo that exercises the probe at the **dramatica-complete →
substrate** boundary — the richer Template layer where DSP, Domain,
Signposts, ThematicPicks, and Story-level fields live. The verifier
emits VerificationReviews targeting records like
`dramatica-complete:DSP_limit`; the probe now reads the underlying
Template records alongside the Dramatic records so the LLM can
comment on whether the verifier's verdicts are well-grounded.

Rocky is the primary target for this probe:

- DSP_limit is NEEDS_WORK 0.33 — the first non-APPROVED DSP_limit
  in the corpus. The declaration is Timelock; the substrate exhibits
  mild Optionlock signals under LT2. This is the highest-signal
  case for the probe to react to — does it propose substrate
  extension (scheduling vocabulary), upper revision (declare
  Optionlock instead), or flag it as honest declaration/taxonomy
  disagreement?
- Story_goal is a trajectory check; DSP_judgment is Good (first for
  MC in the corpus); DSP_outcome is Failure (first in the corpus).
  Non-obvious signals the probe may find worth commenting on.

Usage:
    cd prototype
    export ANTHROPIC_API_KEY=...
    .venv/bin/python3 -m demos.demo_dramatic_reader_model_rocky

    # Dry run (no API call, prints the full prompt):
    .venv/bin/python3 -m demos.demo_dramatic_reader_model_rocky --dry-run

    # Save the full structured result as JSON:
    .venv/bin/python3 -m demos.demo_dramatic_reader_model_rocky \\
        --save-json reader_model_rocky_dramatic_output.json
...
from story_engine.encodings.rocky import ENTITIES, FABULA
from story_engine.encodings.rocky_dramatic import (
    ARGUMENTS, BEATS, CHARACTERS, SCENES, STAKES, THROUGHLINES,
)
from story_engine.encodings.rocky_dramatica_complete import (
    DOMAIN_ASSIGNMENTS,
...
from story_engine.encodings.rocky_lowerings import LOWERINGS
from story_engine.encodings.rocky_dramatica_complete_verification import run as run_rocky_verifier
...
def main() -> int:
    ...
    verifier_results = run_rocky_verifier()

    print("Cross-boundary reader-model probe — Rocky (dramatica-complete)")
