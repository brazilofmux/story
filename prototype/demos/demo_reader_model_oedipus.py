"""
demo_reader_model_oedipus.py — invoke the live reader-model over Oedipus.

Builds a ReaderView on the canonical branch through the anagnorisis
(τ_s ≤ 13) and hands it to Claude Opus 4.6. The encoding's three
descriptions — an anagnorisis texture, a reader-frame on the logical
payload, and an authorial-uncertainty question about parricide/incest
as authored-vs-derived facts — are what the probe reviews, optionally
answers, and optionally edits.

Purpose: forcing function for the inference-model sketch. We want to
see whether the probe reaches for derivation language, proposes
author-vs-derive clarifications, or flags the compound predicates as
structurally suspicious. The question D_parricide_incest_authored_not_
derived is the direct target; the texture and reader-frame give the
probe enough context to answer it meaningfully.

Usage:
    cd prototype
    export ANTHROPIC_API_KEY=...
    .venv/bin/python3 -m demos.demo_reader_model_oedipus
...
def _cli_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "authored in the encoding, including records that "
            "landed via prior walker acceptances — otherwise fresh "
            "LLM output would collide in ordering with accepted "
            "history. Bump this on every new probe round."
        ),
    )
    parser.add_argument(
        "--save-json",
...
def main() -> int:
    ...
    # View spans the full fabula through Oedipus's anagnorisis.
    # up_to_τ_a is set generously past every authored record
    # (including any that landed via prior walker acceptances — the
    # current high-water mark is D_view_cannot_see_τ_d at τ_a=20001;
    # a tight bound would silently hide later walk outputs from the
    # probe's view and turn the demo into a stale snapshot). The
    # default current_τ_a=30000 sits under this bound with room for
    # multiple follow-on rounds; bump both together when a walk
    # advances the encoding's τ_a frontier further.
    view = reader_view(
        branch=CANONICAL,
        events=FABULA,
    ...
    print("Reader-model probe — Oedipus :canonical, through anagnorisis")
