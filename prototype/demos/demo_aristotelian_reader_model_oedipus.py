"""
demo_aristotelian_reader_model_oedipus.py — invoke the Aristotelian
reader-model probe on the Oedipus Tyrannus Aristotelian encoding.

First Aristotelian probe run ever. Baseline invocation: the
encoding was authored directly from Aristotle's own Poetics
worked example (1452a: peripeteia at the messenger's reveal;
1453a: hamartia as missing-the-mark). If the probe can't produce
a clean read here, the probe's surface / prompt / contract is
mis-specified before it ever sees stress.

Predictions from aristotelian-probe-sketch-01 APS6, specific to
this invocation:

- P1: ≥ 80% of the 6 prose reviews earn `approved`, 0 `rejected`.
- P3: `observation_commentaries` is empty — Oedipus verifies with
  zero observations per aristotelian-sketch-01 AA5.
- P4: DialectReading.read_on_terms = "yes"; drift_flagged empty;
  scope_limits_observed and relations_wanted both empty or near-
  empty (no structural pressure to extend — Oedipus IS the
  canonical case).

Usage:
    cd prototype
    export ANTHROPIC_API_KEY=...
    .venv/bin/python3 -m demos.demo_aristotelian_reader_model_oedipus
    ...
    .venv/bin/python3 -m demos.demo_aristotelian_reader_model_oedipus \\
        --save-json reader_model_oedipus_aristotelian_output.json
"""
...
def main() -> int:
    ...
    # observation list. Oedipus is expected to produce zero
    # observations per aristotelian-sketch-01 AA5.
    observations = tuple(verify(
        AR_OEDIPUS_MYTHOS, substrate_events=FABULA,
    ))

    print("Aristotelian reader-model probe — Oedipus Tyrannus")
    print(f"  model: claude-opus-4-6")
    print(f"  effort: {args.effort}")
    print(f"  mythoi: 1 ({AR_OEDIPUS_MYTHOS.id})")
    print(f"  phases: {len(AR_OEDIPUS_MYTHOS.phases)}")
    print(f"  characters: {len(AR_OEDIPUS_MYTHOS.characters)}")
    ...
    if args.save_json:
        context = {
            ...
            "mythoi_ids": [AR_OEDIPUS_MYTHOS.id],
    ...
    if not result.observation_commentaries:
        print("  (none — as expected; Oedipus verifies clean)")
    for c in result.observation_commentaries:
        print()
