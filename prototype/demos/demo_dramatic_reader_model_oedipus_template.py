"""
demo_dramatic_reader_model_oedipus_template.py — invoke the cross-boundary
reader-model probe on the Oedipus Rex dramatica-complete encoding.

Third Template-surface probe run (after Rocky and Macbeth). Oedipus's
substrate is the most identity-resolution-dominant in the corpus: 7
identity-resolution events (pre-plot through anagnorisis) plus 9
rule-derivable emergences of parricide/incest (structurally pre-plot
— the world-facts exist from the crossroads killing and the birth,
but the knowledge convergence is mid-arc). Under LT7 banding the
middle-arc Optionlock signal is carried by the 4 anagnorisis-era
identity-resolutions (τ_s=3, 5, 9, 13).

The probe is invited to comment on:

- Whether DA_mc (APPROVED 0.77 under EK2 — strongest in corpus) is
  well-grounded.
- Story_goal sits at PARTIAL 0.7 — an honest "agent-knowledge
  claim, not world-state claim" finding. Does the probe have a view
  on this goal-identification vs. world-state distinction?
- Whether annotation quality holds across the Lowering surface.
- Any predicate-weakness findings (parallel to Rocky's LT3 dissent).

Usage:
    cd prototype
    export ANTHROPIC_API_KEY=...
    .venv/bin/python3 -m demos.demo_dramatic_reader_model_oedipus_template

    # Dry run (no API call, prints the full prompt):
    .venv/bin/python3 -m demos.demo_dramatic_reader_model_oedipus_template --dry-run

    # Save the full structured result as JSON:
    .venv/bin/python3 -m demos.demo_dramatic_reader_model_oedipus_template \\
        --save-json reader_model_oedipus_dramatica_complete_output.json
...
from story_engine.encodings.oedipus import ENTITIES, FABULA
...
from story_engine.encodings.oedipus_dramatica_complete_verification import run as run_verifier
...
def main() -> int:
    ...
    print("Cross-boundary reader-model probe — Oedipus Rex (dramatica-complete Template surface)")
