...
PH_BEGINNING = ArPhase(
    ...
    annotation=(
        ...
        "opens on Gloucester — forged letter (τ_s=8), shown to "
        "Gloucester (τ_s=9), staged wound (τ_s=11) — with a parallel "
        "branch at τ_s=10 threading Edgar into the deception "
        "(E_edmund_warns_edgar deploys the same forgery-instrument "
        "on a second target); Edgar flees (τ_s=12). The beginning "
        "closes with both plots' "
...
PH_END = ArPhase(
    ...
    annotation=(
        ...
        "Three offstage_death-typed events in this phase "
        "(E_goneril_suicide, E_cordelia_hanged, E_gloucester_dies) "
        "— plus Cornwall's compressed-offstage death in the middle "
        "phase's blinding scene — make Lear the corpus's densest "
        "site of the OQ-AP15 absent-character catharsis shape. "
        "E_lear_dies is onstage (type=death_of_grief) and "
        "E_edmund_dies is death_from_wound."
    ),
)
...
AR_CORDELIA = ArCharacter(
    ...
    hamartia_text=(
        ...
        "status rests on a hamartia-without-encoded-anagnorisis "
        "shape: her early refusal produces the cascade; her death "
        "is the catharsis; but the dialect's anagnorisis apparatus "
        "does not carry her personal recognition — the mythos's "
        "anagnorisis_event_id (E_lear_cordelia_reconcile) and "
        "anagnorisis_character_ref_id (ar_lear) assign the main "
        "recognition to Lear, and no ArAnagnorisisStep names "
        "Cordelia. Sketch-05's A18 `anagnorisis_absent` field "
        "(shipped 2026-04-22, commit 8503d22) structurally carries "
        "the claim: `anagnorisis_absent=True` below marks this "
        "character as a tragic hero whose hamartia produces "
        "catastrophe for others but whose own arc does not contain "
        "a recognition moment. The earlier OQ-LEAR-3 forcing "
        "function (Session 5 surface) is resolved; under Poetics "
        "A4 (1453a), Cordelia qualifies as a tragic character on "
        "hamartia-plus-misfortune grounds, and the recognition-"
        "requirement is now explicitly markable rather than "
        "silently implied."
    ),
    is_tragic_hero=True,
