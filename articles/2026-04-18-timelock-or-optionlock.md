...
## 8. One Reality, or Four?

Compound pressure is the article's current punchline—real stories don't always pick one DSP_limit. There's a stronger version of that claim the engine was built to test but hasn't tested yet.

Dramatica assumes a single Story Mind. Everything the theory adds—Throughlines, Domains, Problems, Goals—sits under the axiom that there IS an objective reality the story inhabits, even if characters don't all know it. Rocky fights Apollo. Oedipus killed his father. Sheppard poisoned Ackroyd. Facts are fixed; what varies is who knows them when.

*Rashomon* refuses the axiom. Four witnesses testify to a killing. Four versions. The substrate encoding I have for Rashomon today uses the project's branch-contested machinery to represent the four testimonies as literal sibling branches—not as "one truth plus three lies" but as four incompatible fabulae a reader is asked to hold simultaneously. I have not yet paired that substrate with a Dramatica upper dialect. I have not yet run it through the probe/verifier loop. But I can say what I expect.

My bet: each of the four contested branches, considered alone, fails the Grand Argument Story form, because the Objective Story Throughline requires fact-fixedness and *Rashomon* refuses that. But the *frame story* at the gate—the woodcutter, the priest, the commoner, trapped by the rainstorm, arguing about what to trust in human nature—that frame may carry a clean Dramatica form. A Timelock (the rain). An Optionlock (they can't leave without resolving something). A Story Goal shaped like *"Can we trust human nature?"* with a payoff the baby in the final scene answers in its own way.

If that reading holds under encoding and probing, *Rashomon* isn't formless. It's *recursive*: broken fragments nested inside a coherent frame. The single-reality axiom would hold at the outer level (the gate) and fail—productively—at the inner (the four testimonies). A verifier that reports the failure as failure would be wrong; a verifier that reports the nesting as a nesting would be doing something the literature doesn't yet have a vocabulary for.

I haven't run any of that. It's the next stress test the substrate's branch-contested design was built for. If the hypothesis lands, the partnership will have something harder to say about what the theory can see and what it can't. If it doesn't land—if *Rashomon* just breaks everything—that's also information.
